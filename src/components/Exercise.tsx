...
export const ExerciseView = ({
  ...
  React.useEffect(() => {
    if ((ex.k === "listen" || ex.k === "dictate") && state.settings.autoAudio)
      speak(ex.say, state.settings.slowAudio);
  ...
  const checkText = () => {
    ...
    } else if (ex.k === "dictate") {
      finish(normalize(text) === normalize(ex.a));
    }
  };
  ...
  return (
    <div className="exercise">
      ...
      {ex.k === "dictate" && (
        <>
          <h2 className="ex-q">Ascolta e scrivi in coreano</h2>
          <button className="big-audio" onClick={() => speak(ex.say, state.settings.slowAudio)}>
            🔊 Riascolta
          </button>
          <input
