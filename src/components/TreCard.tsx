      ...
      <div className="grid-2">
        ...
        <div className="col">
          ...
          <Card className="pad">
            <CardTitle
              emoji="🇰🇷"
              title="Piano per la Corea"
              sub={`${plan.done} parole su ${plan.available} · ${plan.monthsLeft} mesi rimasti`}
            />
            <div className="row-progress">
              <Bar value={(plan.done / plan.available) * 100} color="var(--accent)" />
              <span>{Math.round((plan.done / plan.available) * 100)}%</span>
            </div>
            <p className={`plan-verdict ${plan.onTrack ? "ok" : "late"}`}>
              {plan.onTrack
                ? `In pari con il ritmo di ${plan.monthly} parole al mese.`
                : `Indietro di ${Math.abs(plan.delta)} parole: recupera con una sessione nella banca vocaboli.`}
            </p>
            <button className="btn outline" onClick={() => onNav("vocab")}>
              Studia nuovi vocaboli
            </button>
          </Card>
        </div>
      </div>
