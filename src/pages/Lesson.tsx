...
export const LessonPage = ({
  ...
  const exercises = React.useMemo(
    () => buildLessonExercises(lesson, reviewVocab.slice(0, 6), state.settings.difficulty),
    [lesson.id, reviewVocab, state.settings.difficulty]
  );
  const reviewItems = React.useMemo(
    () => buildReviewExercises(reviewVocab, state.settings.difficulty),
    [lesson.id, reviewVocab, state.settings.difficulty]
  );
  const test = React.useMemo(
    () => buildLessonTest(lesson, state.settings.difficulty),
    [lesson.id, state.settings.difficulty]
  );
