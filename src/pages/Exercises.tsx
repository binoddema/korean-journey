...
type Kind = "ko-it" | "it-ko" | "listen" | "match" | "written" | "write" | "dictate" | "misto";
...
const KINDS: { id: Kind; n: number; label: string; icon: string; desc: string }[] = [
  ...
  { id: "write", n: 6, label: "Scrittura", icon: "⌨️", desc: "Scrivi la parola in coreano" },
  { id: "dictate", n: 7, label: "Dettato", icon: "📝", desc: "Ascolta e scrivi quello che senti" },
  { id: "misto", n: 8, label: "Allenamento misto", icon: "🎲", desc: "Tutti i tipi, secondo la difficoltà" },
];
...
export const Exercises = () => {
  ...
  const items = React.useMemo(
    () => (kind ? buildPractice(pool, kind, writtenPool, state.settings.difficulty, 12) : []),
    [kind, seed]
  );
