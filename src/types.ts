...
export interface Vocab {
  ...
  exKo?: string;
  exIt?: string;
  /** unità della banca vocaboli, se la parola non appartiene a una lezione */
  unit?: string;
  reg?: Register;
...
export interface Settings {
  ...
  reminderTime: string;
  reminderDays: number[];
  /** piano di studio: parole nuove al mese e data obiettivo */
  monthlyWords: number;
  goalDate: string;
}
...
export interface AppState {
  name: string;
  /** primo giorno di utilizzo, per calcolare il ritmo del piano */
  startDate?: string;
  xp: number;
  streak: number;
  bestStreak: number;
...
export interface BankUnit {
  id: string;
  topic: number;
  /** categoria trasversale: "verbi", "gioielleria"… */
  tag?: string;
  title: string;
  emoji: string;
  desc: string;
  /** una parola per riga: ko|romanizzazione|italiano|emoji(opzionale) */
  words: string;
}
